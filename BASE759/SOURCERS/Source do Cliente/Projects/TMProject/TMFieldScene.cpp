...
int TMFieldScene::InitializeScene()
{
	...
	int Painels[] = { 456654, 1919400, 1882000, 1212300, 121200, 188500, 117900, 179500, 192500, 199300, 981300, 98300, 65686, 65692 };
...
int TMFieldScene::OnPacketReconnectParty(MSG_STANDARD* pStd)
{
	...
	if (ReconnectState == 4) {
		m_pMessagePanel->SetMessage("joining party...", 4000);
...
int TMFieldScene::OnPacketUpdateItemLevel(MSG_STANDARD* pStd)
{
	...
	if (g_pClientInfo->Info.chave1 == 0 && g_pClientInfo->Info.Keys[0] >= 0) {
		...
		Text->SetText(strFmt("Choose one of the items to receive"), 0);
...
int TMFieldScene::OnPacketSendInfoPlayer(MSG_STANDARD* pStd) {
	...
	Evo->SetText("Evo:", 0);
	auto Classe = (SText*)m_pControlContainer->FindControl(456602);
...
int TMFieldScene::Affect_Main(unsigned int dwServerTime)
{
	...
	for (int i = 0; i < 32; ++i)
	{
		if ((unsigned char)m_pMyHuman->m_stAffect[i].Type <= 99)
		{
			...
			{
				m_pAffectIcon[i]->m_nPosX = BASE_ScreenResize(220.0f);
				m_pAffectIcon[i]->m_nPosX = ((float)(BASE_ScreenResize(1.0f) + m_pAffectIcon[i]->m_nWidth) * (float)(nAvailCount + 1)) + m_pAffectIcon[i]->m_nPosX;
				m_pAffectIcon[i]->m_nPosY = 0;
...
DWORD WINAPI Guildmark_Download(void* pArg)
{
	...
	strcpy(strURL, "http://26.33.128.59/guilds/img_guilds/"); //Guild Mark Link
