...
int TMSelectServerScene::InitializeScene()
{
	...
	m_Logo1 = static_cast<SPanel*>(m_pControlContainer->FindControl(311));
	m_Logo2 = static_cast<SPanel*>(m_pControlContainer->FindControl(312));
	...
	/*if (g_pDevice->m_dwScreenWidth == 800 || g_pDevice->m_dwScreenWidth == 640 || g_pDevice->m_dwScreenWidth == 1024 || g_pDevice->m_dwScreenWidth == 1280) {
		m_Logo1->SetVisible(1);
		m_Logo2->SetVisible(0);
	}
	else {
		m_Logo1->SetVisible(0);
		m_Logo2->SetVisible(1);
		m_Logo2->m_nHeight = g_pDevice->m_dwScreenHeight;
		m_Logo2->m_nWidth = g_pDevice->m_dwScreenWidth;
	}*/
	...
	if (g_pClientInfo->Info.LoginDone == 1) {

		int nServerGroupIndex = g_pClientInfo->Info.ServerGroupIndex;
		int nServerIndex = g_pClientInfo->Info.ServerIndex;
		...
		auto LOGIN = (SEditableText*)m_pControlContainer->FindControl(65871);
		LOGIN->SetText(strFmt("%s", RecName));
		auto SENHA = (SEditableText*)m_pControlContainer->FindControl(65872);
		SENHA->SetText(strFmt("%s", RecPass));
		...
		HANDLE threadHandle;
		DWORD threadID;
		threadHandle = CreateThread(NULL, 0, Timer, NULL, 0, &threadID);
	}
...
int TMSelectServerScene::OnControlEvent(unsigned int idwControlID, unsigned int idwEvent)
{
	...
	case B_SERVER_SEL_OK:
	{
		...
		g_pClientInfo->Info.ServerGroupIndex = nServerGroupIndex;
		g_pClientInfo->Info.ServerIndex = nServerIndex;
	...
	case B_LOGIN_OK:
	{
		...
		sprintf_s(RecName, "%s", pEditID->GetText());
		sprintf_s(RecPass, "%s", pEditPassword->GetText());
...
{
	if (ReconnectTimer == 5 && ReconnectState == 1) {
		Reconnecting_Login();
		ReconnectTimer = 0;
	}
