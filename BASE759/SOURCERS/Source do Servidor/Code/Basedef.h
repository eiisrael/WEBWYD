...
#define Aposta_250bi            75
#define Aposta_1kbi	   			76
#define BattleNPC               78
#define NPC_KBI                 83 //entregue 1kbi = 5641 recebe um cupom_kbi ID: 5664
#define AMUNRRA_COMBINEHP       84
#define AMUNRRA_COMBINEMP       91
...
#define EFV3            stEffect[2].cValue
	int getSanc()
	{
		if (sIndex >= 2330 && sIndex < 2390)
			return 0;

		unsigned char sanc = 0;

		if (stEffect[0].cEffect >= 116 && stEffect[0].cEffect <= 126)
			sanc = stEffect[0].cValue;

		else if (stEffect[1].cEffect >= 116 && stEffect[1].cEffect <= 126)
			sanc = stEffect[1].cValue;

		else if (stEffect[2].cEffect >= 116 && stEffect[2].cEffect <= 126)
			sanc = stEffect[2].cValue;

		else if (stEffect[0].cEffect == 43 || stEffect[1].cEffect == 43 || stEffect[2].cEffect == 43)
		{
			if (stEffect[0].cEffect == 43)
				sanc = stEffect[0].cValue;
			else if (stEffect[1].cEffect == 43)
				sanc = stEffect[1].cValue;
			else
				sanc = stEffect[2].cValue;
		}
	...
	void SetAmount(int Amount)
	{
		...
		if (this->stEffect[0].cEffect == 61 || this->stEffect[1].cEffect == 61 || this->stEffect[2].cEffect == 61)
		{
			if (this->stEffect[0].cEffect == 61)
				this->stEffect[0].cValue = Amount;

			else if (this->stEffect[1].cEffect == 61)
				this->stEffect[1].cValue = Amount;

			else
				this->stEffect[2].cValue = Amount;
		}
		else if (this->stEffect[0].cEffect == 0 || this->stEffect[1].cEffect == 0 || this->stEffect[2].cEffect == 0)
		{

			if (this->stEffect[0].cEffect == 0)
			{
				this->stEffect[0].cEffect = 61;
				this->stEffect[0].cValue = Amount;
			}
			else if (this->stEffect[1].cEffect == 0)
			{

				this->stEffect[1].cEffect = 61;
				this->stEffect[1].cValue = Amount;

			}
			else
			{
				this->stEffect[2].cEffect = 61;
				this->stEffect[2].cValue = Amount;
			}
		}
	}
	int Amount()
	{
		...
		{
			if (this->stEffect[0].cEffect == 61)
				amount = this->stEffect[0].cValue;
			else if (stEffect[1].cEffect == 61)
				amount = this->stEffect[1].cValue;
			else
				amount = this->stEffect[2].cValue;
		}

		return amount;
	}
	short totalAmount(STRUCT_ITEM* other)
	{
		int amount_1 = 1, amount_2 = 1;
		if (Amount() == 0)
			amount_1 = 1;
		else
			amount_1 = Amount();
		...
		if (other->Amount() == 0)
			...
			amount_2 = other->Amount();

		return amount_1 + amount_2;
	}
	//TODO: reduz o amount do item
	void AmountMinus()
	{
	...
	//TODO: reduz o amount do item por quantidade
	void AmountMinus(int Count)
	{
		int amount = this->Amount();
		if (amount > Count)
			this->SetAmount(amount - Count);
		else
			this->clear();
	}
	void PutEffect(short id, short value, short local)
	{
		int i = local;
		for (; i < 3; i++)
		{
			if (this->stEffect[i].cEffect == 0 || this->stEffect[i].cEffect == id)
			{
				this->stEffect[i].cEffect = (unsigned char)id;
				this->stEffect[i].cValue = (unsigned char)value;
				break;
			}
		}
	}
...
const short _MSG_DBAccountLoginCreateAccount_Pass = (40 | FLAG_DB2GAME);
const short _MSG_DBAccountLoginCreateAccount_PassFail = (41 | FLAG_DB2GAME);
