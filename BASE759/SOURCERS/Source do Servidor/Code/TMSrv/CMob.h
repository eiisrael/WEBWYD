...
public:
	...
	//cod aposta pvp 
	int HasItemMount(int Item, int Level)
	{
		int x = 0;
		for (int i = 0; i < this->MaxCarry; ++i) {

			if (this->MOB.Carry[i].sIndex == Item && this->MOB.Carry[i].stEffect[1].cEffect >= Level)
				x++;

		}
		return x;
	}
	//cod aposta pvp  250bi
	int HasItem(int Item, int Quantidade = 0)
	{
		int x = 0;

		for (int i = 0; i < this->MaxCarry; ++i) {

			if (this->MOB.Carry[i].sIndex == Item && this->MOB.Carry[i].Amount() >= Quantidade)
				x++;
	...
	int CarryFree(int slot)
	{
		int z = 0;
		for (int y = 0; y < this->MaxCarry; ++y) {

			if (this->MOB.Carry[y].sIndex == 0)
				z++;
		}
		return z;
	}
