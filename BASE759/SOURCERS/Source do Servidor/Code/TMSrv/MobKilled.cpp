...
void MobKilled(int target, int conn, int PosX, int PosY)
{
	...
	//cod aposta pvp 
	if (target > 0 && target < MAX_USER && Aposta == 2 && pUser[target].ApostaStart && pUser[conn].ApostaStart) {


		pMob[target].MOB.CurrentScore.Hp = 2;


		pUser[target].ApostaStart = 0;
		pUser[conn].ApostaStart = 0;
		pUser[conn].ApostaWinner++;

		//sprintf(temp, "O Player %s é o vencedor da aposta(5b) o Player %s Fraco", pMob[conn].MOB.MobName, pMob[target].MOB.MobName);
		sprintf(temp, "O Player %s é o vencedor da aposta(x1) Contra o Player %s Fraco", pMob[conn].MOB.MobName, pMob[target].MOB.MobName);
		SendNotice(temp);
		DoRecall(conn);
		DoRecall(target);
		return;
	}
	if (target > 0 && target < MAX_USER && Apostakb == 4 && pUser[target].ApostaStartkb && pUser[conn].ApostaStartkb) {


		pMob[target].MOB.CurrentScore.Hp = 2;


		pUser[target].ApostaStartkb = 0;
		pUser[conn].ApostaStartkb = 0;
		pUser[conn].ApostaWinnerkb++;

		//sprintf(temp, "O Player %s é o vencedor da aposta(250b) contra o Player %s Fraco", pMob[conn].MOB.MobName, pMob[target].MOB.MobName);
		sprintf(temp, "O Player %s é o vencedor da aposta(X1) Contra o Player %s Fraco", pMob[conn].MOB.MobName, pMob[target].MOB.MobName);
		SendNotice(temp);
		DoRecall(conn);
		DoRecall(target);
	...
	if (target > 0 && target < MAX_USER && pUser[target].BattleChall == 2 && pUser[conn].BattleChall == 2) {

		pMob[target].MOB.CurrentScore.Hp = 2;
		pUser[target].BattleChall = 0;
		DoRecall(target);
		BattleChallP--;
		...
		sprintf_s(temp, "[BattlePVP] %s foi Morto por %s no ", pMob[target].MOB.MobName, pMob[conn].MOB.MobName);
		SendNotice(temp);
		return;
	...
	//cod aposta pvp 
	if (RvRState && pMob[target].TargetX >= 1020 && pMob[target].TargetY >= 1916 && pMob[target].TargetX <= 1286 && pMob[target].TargetY <= 2178 && target > 0 && target < MAX_USER) {

		ZeroMemory(&temp, sizeof(temp));

		sprintf(temp, "[RVR] %s foi morto por %s", pMob[target].MOB.MobName, pMob[conn].MOB.MobName);
		SendNotice(temp);
	}
			...
			{		
				if ((conn > 0 && conn < MAX_USER) && (target > 0 && target < MAX_USER)) {
					...
					if (pMob[target].TargetX >= 140 && pMob[target].TargetY >= 4000 && pMob[target].TargetX <= 200 && pMob[target].TargetY <= 4100)
					{
						...
						STRUCT_ITEM presente = { 4026, 61, 20, 0 };
...
#pragma endregion
#pragma region Invasao - Boss

						if ((!strcmp(pMob[target].MOB.MobName, "QuatroPatas")) || (!strcmp(pMob[target].MOB.MobName, "QuatroPatas"))
							|| (!strcmp(pMob[target].MOB.MobName, "QuatroPatas")) || (!strcmp(pMob[target].MOB.MobName, "QuatroPatas")))
						{
							if (g_quests.HouseNLeft >= 100) return;


							SendCounterMob(conn, g_quests.HouseNLeft, 100);

							g_quests.HouseNLeft++;

							if (g_quests.HouseNLeft == 100)
							{
								if (g_quests.HouseNStatus == false)
								{
									CreateMob("INFAMUS", 3635, 3051, "npc", 0);

									if ((pMob[conn].TargetX / 128) == 36 || (pMob[conn].TargetY / 128) == 30)
										SendClientMessage(conn, "Drop seu Item o BOSS nasceu no Gelo.");
						...
						if (!strcmp(pMob[target].MOB.MobName, "INFAMUS"))
						{
							int rand_ = rand() % 100;
							STRUCT_ITEM Item;
							memset(&Item, 0, sizeof(STRUCT_ITEM));
							...
							if (rand_ <= 10)
								Item.sIndex = 2317;// ovo de dragao

							else if (rand_ <= 30)
							{
								Item.sIndex = 3438; //acelerador de crescimento
							...
							{
								Item.sIndex = 2407; //Amago de dragão
								Item.stEffect[0].cEffect = EF_AMOUNT;
								Item.stEffect[0].cValue = 0;
							}
							else
							{
								Item.sIndex = 2416; //amago de grifo sangrento
								Item.stEffect[0].cEffect = EF_AMOUNT;
								Item.stEffect[0].cValue = 0;
							}
