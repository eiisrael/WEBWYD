...
#pragma endregion
#pragma region MSG_DBAccountLoginCreateAccount_Pass
		case _MSG_DBAccountLoginCreateAccount_Pass:
		{
			SendClientMessage(conn, "Conta criada com sucesso!");
			pUser[conn].cSock.SendMessageA();
			CloseUser(conn);
		} break;
#pragma endregion
#pragma region MSG_DBAccountLoginCreateAccount_PassFail
		case _MSG_DBAccountLoginCreateAccount_PassFail:
		{
			SendClientMessage(conn, "Aguarde 1 minuto para criar uma nova conta.");
			pUser[conn].cSock.SendMessageA();
			CloseUser(conn);
		} break;
#pragma endregion
