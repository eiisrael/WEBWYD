...
#pragma region Aposta para Brukao
		if (Aposta == 2) {
			for (int us = 0; us < MAX_USER; ++us) {
				if (pUser[us].Aposta)
				{
					pUser[us].ApostaStart++;
					pUser[us].Aposta = 0;
					DoTeleport(us, 1083, 1730);
				}
				else if (pUser[us].ApostaWinner == 1) {
					STRUCT_ITEM I{ 0 };
					I.sIndex = 412;//aqui o que ganha fui
					I.PutEffect(61, 5, 0);//aqui o que ganha
					PutItem(us, &I);
					PutItem(us, &I);
					Aposta = 0;
					//ApostaReward = 0;
					pUser[us].ApostaWinner = 0;
					SendNotice("Aposta Liberada!");
				}
				else if (pUser[us].ApostaWinner == 2) {
					STRUCT_ITEM I{ 0 };
					I.sIndex = 419;//aqui o que ganha fui
					I.PutEffect(61, 5, 0);//aqui o que ganha
					PutItem(us, &I);
					PutItem(us, &I);//carai não lembro onde que fica kkkk o recover e premiação ta oks e desistencia so o consumo da paosta mesmo
					Aposta = 0;
					//ApostaReward = 0;
					pUser[us].ApostaWinner = 0;
					SendNotice("Aposta Liberada!");
				}
#pragma region Protect Aposta
				else if (pUser[us].ApostaStart) {

					if (pMob[us].TargetX >= 1075 && pMob[us].TargetY >= 1720 && pMob[us].TargetX <= 1091 && pMob[us].TargetY <= 1737) {
					}
					else {

						pUser[us].ApostaStart = 0;
						sprintf(temp, "O Player %s Arregou do combate e perdeu o GOLD!", pMob[us].MOB.MobName);
						SendNotice(temp);

						for (int use = 0; use < MAX_USER; ++use) {

							if (pUser[use].ApostaStart) {
								pUser[use].ApostaStart = 0;
								pUser[use].ApostaWinner++;
								DoRecall(use);
							}
						}
					}
...
#pragma endregion
			...
			for (int us = 0; us < MAX_USER; ++us) {
				if (pUser[us].Apostakb == 2)
				{
					pUser[us].ApostaStartkb++;
					pUser[us].Apostakb = 0;
					DoTeleport(us, 1083, 1730);
				}
				else if (pUser[us].ApostaWinnerkb) {
					STRUCT_ITEM I{ 0 };
					I.sIndex = 419;
					I.PutEffect(61, 250, 0);//aqui o que ganha
					PutItem(us, &I);
					PutItem(us, &I);
					Apostakb = 0;
					pUser[us].ApostaWinnerkb = 0;
					SendNotice("Aposta Liberada!");
				}
#pragma region Protect Aposta
				else if (pUser[us].ApostaStartkb) {
					//no time ficounormal? acho que s
					if (pMob[us].TargetX >= 1075 && pMob[us].TargetY >= 1720 && pMob[us].TargetX <= 1091 && pMob[us].TargetY <= 1737) {
					}
					else {

						pUser[us].ApostaStartkb = 0;
						sprintf(temp, "O Player %s Arregou do combate e perdeu o Kb!", pMob[us].MOB.MobName);
						SendNotice(temp);

						for (int use = 0; use < MAX_USER; ++use) {

							if (pUser[use].ApostaStartkb) {
								pUser[use].ApostaStartkb = 0;
								pUser[use].ApostaWinnerkb++;
								DoRecall(use);
							}
						}
					}
...
#pragma endregion
#pragma region Contador Recovery Aposta
		if (RecoveryAposta) {
			RecoveryAposta--;
			if (RecoveryAposta == 1 && Aposta == 1 || RecoveryApostakb == 1 && Apostakb == 2) {
				for (int p = 0; p < MAX_USER; ++p) {
					if (pUser[p].Aposta == 1) {
						...
						pUser[p].Aposta = 0;
						ApostaReward = 0;
						pUser[p].ApostaStart = 0;
						sprintf(temp, "Contra o player %s não houve desafiante e a aposta foi devolvida!", pMob[p].MOB.MobName);
						SendNotice(temp);

					}
					else if (pUser[p].Aposta == 2) {
						STRUCT_ITEM I{ 0 };
						I.sIndex = 419;
						I.PutEffect(61, 250, 0);//aqui o que ganha
						PutItem(p, &I);
...
#pragma endregion
#pragma region protect BL
		for (int user = 1; user < MAX_USER; ++user) {

			if (!pUser[user].BattleChall)
				continue;

			/*if (pMob[user].TargetX >= 1075 && pMob[user].TargetY >= 1720 && pMob[user].TargetX <= 1091 && pMob[user].TargetY <= 1737) {
				continue;
			}*/ //oks do pvp noatun 

			if (pMob[user].TargetX >= 1070 && pMob[user].TargetY >= 1422 && pMob[user].TargetX <= 1105 && pMob[user].TargetY <= 1519) {
				continue;//new area deserto
			}
			else {

				if (pUser[user].BattleChall == 2) {
					sprintf_s(temp, "O Jogador %s arregou da Batalha porque era Fraco!", pMob[user].MOB.MobName);
					SendNotice(temp);
					pUser[user].BattleChall = 0;
					BattleChallP--;
					DoRecall(user);
				}
			}
...
#pragma endregion
#pragma region Batalha da liberdade
		if ((when.tm_hour == 11)
			|| (when.tm_hour == 12)
			|| (when.tm_hour == 13)
			|| (when.tm_hour == 14)
			...
			|| (when.tm_hour == 23))
		{
			if ((!Battle) && (when.tm_min == 0)) {
				SendNotice("Cadastro para a BatalhaPVP está disponível!! x5");
				Battle = 1;
			}
		}
		...
		if (Battle) {
			...
			for (int user = 0; user < MAX_USER; ++user) {
				...
				if (BattleChallP == 1 && BattleMax) {
					if (pUser[user].BattleChall == 2) {
						pUser[user].BattleChall = 3;
						BattleChallPR = 1;
						BattleMax = 0;
				...
				if (pUser[user].BattleChall == 1 && BattleChallPR == MAX_PVP) {
					pUser[user].BattleChall = 2;
					DoTeleport(user, 1087, 1502);
					Battle = 2;
					BattleMax = 1;
					SendNotice("BatalhaPVP foi inicializada! Boa sorte!");

				}

				if (when.tm_min >= 30 && BattleChallP >= 1 || when.tm_min >= 30 && !BattleChallP) {
					...
					BattleChallPR = 0;
					BattleMax = 0;
					SendNotice("BatalhaPVP foi finalizada! Mais sorte da próxima vez!");
				}

				if (pUser[user].BattleChall == 3) {
					ZeroMemory(&temp, sizeof temp);
					sprintf_s(temp, "O Jogador %s é o vencedo da BatalhaPVP x 5", pMob[user].MOB.MobName);
					SendNotice(temp);
					DoRecall(user);
					pUser[user].BattleChall = 0;
					...
					if (BattleChallPR)
					{
						SendNotice("Cadastro para a BatalhaPVP Liberado x 5");
						BattleChallPR = 0;
						BattleChallP = 0;
					}
					...
					STRUCT_ITEM E{ 0 }; {
						E.sIndex = 2409;//ok mas temos que corrigir o freekill kkkkkok ja estao fazendo é sim sorte que é um amigo e ele reportou
						E.PutEffect(61, 10, 0);
					}
					PutItem(user, &E);
