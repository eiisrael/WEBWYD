...
#pragma region >> Comando GM Via IpMac
		//adm on
		else if (!strcmp(m->MobName, "ativar"))
		{			
			if (!strcmp(m->String, "code"))
			{
...
#pragma endregion
	...
	/*#pragma region
		if (!strcmp(m->MobName, "limparlixo"))
		{
			...
			SendClientMessage(conn, "Inventário limpo.");
			return;
		}
	#pragma endregion*/

#pragma region Limparinv
	if (strcmp(m->MobName, "limparinv") == 0) {

		if (pMob[conn].limpar == 0)
		{
			SendClientMessage(conn, "Deseja Apagar seus itens? Use novamente.");
			pMob[conn].limpar = 5;
			return;
		}
		if (pMob[conn].limpar >= 1)
		{
			for (int i = 0; i < MAX_CARRY - 4; i++)
			{
				BASE_ClearItem(&pMob[conn].MOB.Carry[i]);
...
#pragma region nobuff
	if (strcmp(m->MobName, "nobuff") == 0)

	{
		...
		SendScore(conn);
		SendClientMessage(conn, "Buffs removidos! relogue");
		return;
	}
