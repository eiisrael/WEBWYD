...
#pragma region Set npcMode Base
	...
	if (npcMerc == 84)
		npcMode = AMUNRRA_COMBINEHP;

	if (npcMerc == 91)
		npcMode = AMUNRRA_COMBINEMP;
...
#pragma endregion
#pragma region AMUNRRA_COMBINEHP
	case AMUNRRA_COMBINEHP:
	{
		if (pMob[conn].MOB.Coin < 2000000000)
		{
			SendClientMessage(conn, "Traga 2 Milhões de Gold + 1 Folha de Mandrágora, 1 Coracão da Nyerdes e 1 Joia da Escuridão..");
			break;
		}

		int Materiais[3] = { 0, 0, 0 };

		for (int i = 0; i < pMob[conn].MaxCarry; i++)
		{
			if (pMob[conn].MOB.Carry[i].sIndex < 770 || pMob[conn].MOB.Carry[i].sIndex > 772)
				continue;

			int Id = pMob[conn].MOB.Carry[i].sIndex - 770;

			Materiais[Id]++;
		}

		if (Materiais[0] < 1 || Materiais[1] < 1 || Materiais[2] < 1)
		{
			SendClientMessage(conn, "Traga os 3 Materiais.");
			break;
		}
		...
		for (int x = 0; x < 3; x++)
		{
			for (int i = 0; i < pMob[conn].MaxCarry && Materiais[x] > 0; i++)
			{
				if (pMob[conn].MOB.Carry[i].sIndex == (770 + x))
				{
					memset(&pMob[conn].MOB.Carry[i], 0, sizeof(STRUCT_ITEM));
					SendItem(conn, ITEM_PLACE_CARRY, i, &pMob[conn].MOB.Carry[i]);
					Materiais[x]--;
		...
		Item.sIndex = 3464; //Amunrra
		Item.stEffect[0].cEffect = 69;
		Item.stEffect[0].cValue = 10;
		Item.stEffect[1].cEffect = 2;
		Item.stEffect[1].cValue = 120;
		Item.stEffect[2].cEffect = 43;
		Item.stEffect[2].cValue = rand() % 4;

		PutItem(conn, &Item);

		SendClientMessage(conn, "Combinacão criada com Sucesso.");

		sprintf(temp, "etc,arzan_dragon snowimp create name:%s level:%d", pMob[conn].MOB.MobName, pMob[conn].MOB.BaseScore.Level);
		Log(temp, "-system", 0);
	}break;
#pragma endregion
#pragma region AMUNRRA_COMBINEMP
	case AMUNRRA_COMBINEMP:
	{
		if (pMob[conn].MOB.Coin < 2000000000)
		{
			SendClientMessage(conn, "Traga 2 Milhões de Gold + 1 Folha de Mandrágora, 1 Coracão da Nyerdes e 1 Joia da Escuridão..");
		...
		Item.sIndex = 3464; //Amunrra
		Item.stEffect[0].cEffect = 70;
		Item.stEffect[0].cValue = 10;
		Item.stEffect[1].cEffect = 60;
		Item.stEffect[1].cValue = 14;
		Item.stEffect[2].cEffect = 43;
		Item.stEffect[2].cValue = rand() % 4;
...
#pragma region NPC_KBI
	case NPC_KBI:
	{
		...
		if (i != pMob[conn].MaxCarry)
		{
			...
			{
				SendClientMessage(conn, "Traga-me 2 Bilhão pegarei 1 só em troca de um Item");
				return;
			}
			if (amount > 1)
			...
			if (_rd < 40)
			{
				Item.sIndex = 3467; // bolsa andarilho

			}

			else
			{
				Item.sIndex = 5709; //poção divina 1 dia
			}
...
#pragma endregion
#pragma region Battle NPC
	case BattleNPC:
	{
		if (pUser[conn].BattleChall) {
			SendClientMessage(conn, "Você já realizou o cadastro");
			break;
		}
		else {
			...
			if (Battle) {
				...
				/* if (BattleVerify(conn)) {
					 break;
				 }*/

				ZeroMemory(&temp, sizeof temp);

				if (BattleChallPR == MAX_PVP) {
					SendClientMessage(conn, "Limite de participantes atingido");
					break;
				}

				sprintf_s(temp, "O Jogador %s cadastrou-se para a BatalhaPVP", pMob[conn].MOB.MobName);
				SendNotice(temp);
				pUser[conn].BattleChall = 1;
				BattleChallP++;
				BattleChallPR++;
				break;
			}
			else {
				SendSay(npcIndex, "Cadastre-se a cada 1 hora");
				break;
			}
		}
	}
...
#pragma endregion
#pragma region Aposta_1kbi
	case Aposta_1kbi: {

		if (pUser[conn].Aposta || pUser[conn].Apostakb) {
			SendClientMessage(conn, "Você já realizou o cadastro.");
			break;
		}
		if (Apostakb == 4 || Aposta == 1)
		{

			SendClientMessage(conn, "Limite de participantes atingido aguarde 15m!");
			break;
		}
		bool Tru = false;
		if (pMob[conn].HasItem(419, 250) > 0) {
			pUser[conn].Aposta = 1;
			Apostakb += 2;
			pUser[conn].Apostakb = 2;
			sprintf(temp, "O Player %s registrou-se x1 na Aposta(Duelo)!", pMob[conn].MOB.MobName);
			//sprintf(temp, "O Player %s registrou-se x1 Aposta_250(NPC em Noatum)!", pMob[conn].MOB.MobName);
			SendNotice(temp);
			RecoveryApostakb = 900;
			Tru = true;
			...
			sprintf(temp, "..\\..\\Common\\LogAposta\\KB_%s%d%d%d%d%d.txt", pUser[conn].AccountName, when.tm_mday, when.tm_mon + 1, when.tm_hour, when.tm_min, when.tm_sec);
			PG = fopen(temp, "wt");
			fprintf(PG, "o Player %s Registrou-se na aposta(x1) no dia %d/%d às %d:%d", pMob[conn].MOB.MobName, when.tm_mday, when.tm_mon + 1, when.tm_hour, when.tm_min);
			//fprintf(PG, "o Player %s Registrou-se na aposta(250b) no dia %d/%d às %d:%d", pMob[conn].MOB.MobName, when.tm_mday, when.tm_mon + 1, when.tm_hour, when.tm_min);
			fclose(PG);
		}
		else {
			sprintf(temp, "Traga-me %s(250) para participar da Aposta", g_pItemList[419].Name);
			SendClientMessage(conn, temp);
			break;
		...
		if (Tru == true)
		{
			for (int i = 0; i < 60; ++i) {//pera ai ligar o srv sem o npc rapido por causa da torre go

				if (pMob[conn].MOB.Carry[i].sIndex == 419) {
					pMob[conn].MOB.Carry[i].AmountMinus(250);
					/* pMob[conn].MOB.Carry[i].AmountMinus();*/
					SendItem(conn, ITEM_PLACE_CARRY, i, &pMob[conn].MOB.Carry[i]);//queres ver o que acontece ou ja entendeu?
					break;
				}
			}
		}
...
#pragma region Aposta_250bi = 5 bi
	case Aposta_250bi:
	{
		...
		if (Apostakb == 2 || Aposta == 2)//se houver 1 cadastro apenas um pode continuar 
		{
		...
		bool Tru = false;
		if (pMob[conn].HasItem(412, 5) > 0) {
			pUser[conn].Aposta = 1;
			Aposta += 1;
			sprintf(temp, "O Player %s registrou-se para o x1 na ApostaReal(Duelo)!", pMob[conn].MOB.MobName);
			//sprintf(temp, "O Player %s registrou-se para o x1 ApostaReal(5Bi)!", pMob[conn].MOB.MobName);
			SendNotice(temp);
			RecoveryAposta = 900;
			Tru = true;
			...
			sprintf(temp, "..\\..\\Common\\LogAposta\\%s%d%d%d%d%d.txt", pUser[conn].AccountName, when.tm_mday, when.tm_mon + 1, when.tm_hour, when.tm_min, when.tm_sec);
			PG = fopen(temp, "wt");
			fprintf(PG, "o Player %s Registrou-se na aposta no dia %d/%d às %d:%d", pMob[conn].MOB.MobName, when.tm_mday, when.tm_mon + 1, when.tm_hour, when.tm_min);
			fclose(PG);
		}
		else {

			sprintf(temp, "Traga-me %s(5) para participar da Aposta", g_pItemList[412].Name);
			SendClientMessage(conn, temp);
			break;
		...
		{//agor
			for (int i = 0; i < 60; ++i) {
				...
				if (pMob[conn].MOB.Carry[i].sIndex == 412 && pMob[conn].MOB.Carry[i].Amount() >= 5) {
					pMob[conn].MOB.Carry[i].AmountMinus(5);
					SendItem(conn, ITEM_PLACE_CARRY, i, &pMob[conn].MOB.Carry[i]);
